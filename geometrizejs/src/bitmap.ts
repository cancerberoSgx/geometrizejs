 ...
 * 
 * It supports the concept of offset, this is a inner adjustable rectangular area and methods 
 * to convert from absolute to coordinates relative to the offset.
 */
export declare class Bitmap {
  ...
  /**
   * Sets the bitmap offset, this is, a region inside relative to which pixel read and write operations are
   * made. Calling this method without parameters will remove the offset and reset to default behavior.
   * 
   */
  ...
  /**
   * Creates a new bitmap from the supplied native raw byte array. Useful for target language consumers that
   * don't have access to Bytes haxe standar library class. 
   * @param w   The width of the bitmap.
   * @param h   The height of the bitmap.
   * @param bytes The byte data to fill the bitmap with, must be width * height * depth long.
   * @return  The new bitmap.
...
/**
 * Contains the raw bitmaps as a field. 
 */
